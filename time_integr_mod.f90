...
! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! TimeIntegratorORK_SH_MPI_DGV 
! 
! This is an MPI verstion of the subroutine above
! This subroutine implements Optimal Runge Kutta integration in the spatially homogeneous case. 
! 
! the only change is the call of the MPI-enhanced spatial operator.. all the rest is the same... 
! The spatial operator has an MPI fork
! It is dependent on the main program: uses variables from the 
! commvar 
!
! the called subroutines look up varaibles in commvar
...
SUBROUTINE TimeIntegratorORK_SH_MPI_DGV(f,dt)
...
SELECT CASE (rkmts)
	!!! RUNGE-KUTTA FIRST ORDER (EULER METHOD)
	CASE (1) 
		CALL SpatialOperSH_MPI_DGV (f,frhs1,dt)   ! HAS MPI FORK INSIDE
		f = f + frhs1
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
	!!! RUNGE KUTTA SECOND ORDER (OPTIMAL 2ND ORDER METHOD)
	CASE (2)
	  ...
	  !!!!! BEGIN THE RUNGE-KUTTA STEP !!!!!
	  !!! t_1:
	  CALL SpatialOperSH_MPI_DGV (f, frhs1, dt)
      !!! t_2: 
	  CALL SpatialOperSH_MPI_DGV (f + frhs1*2/3.0_DP, ft2, dt)
	  !!!!                     
	   f = f + frhs1/4.0_DP + ft2*3.0_DP/4
	  !!!! 	
	  deallocate(ft2)
	...
	CASE (3)
	  ...
	  CALL SpatialOperSH_MPI_DGV (f, frhs1, dt)                       ! HAS MPI FORK INSIDE
	  !!! t_2:
	  CALL SpatialOperSH_MPI_DGV (f + frhs1/2.0_DP, ft2, dt)          ! HAS MPI FORK INSIDE
	  !!! t_3:
	  CALL SpatialOperSH_MPI_DGV (f + ft2*3.0_DP/4, ft3, dt)          ! HAS MPI FORK INSIDE
	...
	CASE (4)
	  ...
	  CALL SpatialOperSH_MPI_DGV (f + frhs1/2.0_DP, ft2, dt)                  ! HAS MPI FORK INSIDE
	  !!! t_3:
	  CALL SpatialOperSH_MPI_DGV (f + frhs1/4.0_DP + ft2/4.0_DP, ft3, dt)     ! HAS MPI FORK INSIDE
	  !!! t_4:
	  CALL SpatialOperSH_MPI_DGV (f - ft2 + ft3*2, ft2, dt)                   ! HAS MPI FORK INSIDE
	  f = f + frhs1/6.0_DP + ft3*2/3.0_DP + ft2/6.0_DP  ! :2nd componenent is re-used for 4th component!
	  deallocate (ft2,ft3)
	!!! RUNGE-KUTTA FIFTH ORDER (RALSTON AND RABINOWITZ - COMPATIBLE WITH THE FOURTH ORDER)
	CASE (5)
	  ...
	  print *, "3"
	  !!! t_4: 
	  CALL SpatialOperSH_MPI_DGV (f - ft2 + ft3*2.0_DP, ft4, dt)                         ! HAS MPI FORK INSIDE
	  print *, "4"
	  !!! WE NOW INTRODUCE SOME MEMORY SAVINGS AND REPLACE FT3 WITH -FT2/5_DP+FT3*546/625_DP THAT IS GOING TO BE USED IN STEP 6
	  ft3 = -ft2/5.0_DP+ft3*546/625.0_DP
      !!! t_5:
	  !!! ON THIS STEP, WE RE-USE ft2 TO TAKE PLACE OF ft5
	  CALL SpatialOperSH_MPI_DGV (f + frhs1*7.0_DP/27.0_DP + ft2*10.0_DP/27.0_DP + ft4/27.0_DP, ft2, dt) ! HAS MPI FORK INSIDE
	  print *, "5"
	  !!! t_6: 
	  !!! ON THIS STEP, WE RE-USE ft3 TO TAKE PLACE OF ft6
	  CALL SpatialOperSH_MPI_DGV (f + frhs1*28.0_DP/625.0_DP + ft3 + ft4*54/625.0_DP - ft2*378/625.0_DP, ft3, dt) ! HAS MPI FORK INSIDE
	  print *, "6"
	  f = f + frhs1/24.0_DP + ft4*5.0_DP/48.0_DP + ft2*27.0_DP/56.0_DP + ft3*125/336.0_DP
	  deallocate (ft2,ft3,ft4)
	CASE default
...
END SUBROUTINE TimeIntegratorORK_SH_MPI_DGV


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! TimeIntegratorMTS_HOV1D
!
...
USE commvar, only: f1,f2,f3,f4,frhs1,frhs2,frhs3,frhs4, &
                    rkmts,frhs5,e_time
USE spat_oper_mod                    
...
SELECT CASE (rkmts)
	...
	END SELECT
    frhs1 = frhs1/dt
    e_time = e_time + dt ! advance the time for one step
END SUBROUTINE TimeIntegratorMTS_SH_DGV

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! TimeIntegratorMTS_MPI_SH_MPI_DGV
!
! Modified by  Alex 10/21/2011
! 
! This subroutine implements Multiple Step Time integration. 
! 
!  This is an MPI analog of the above subroutine. SpatialOperSH_MPI_DGV has an MPI fork in it.
! 
! It is dependent on the main program: uses variables from the 
! common_variables_mod 
!
...
SUBROUTINE TimeIntegratorMTS_SH_MPI_DGV(f,dt)
...
SELECT CASE (rkmts)
    ...
    frhs1 = frhs1/dt
    e_time = e_time + dt ! advance the time for one step
END SUBROUTINE TimeIntegratorMTS_SH_MPI_DGV
...
USE spat_oper_mod
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!! DIAGNOSTIC A (use together with B)
USE dgvtools_mod, only: RecordMomDerivRelaxTimes_DGV
!! END DIAGNOSTIC A 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
...
!!! MPI FORK IS USED THERE !!!
 call TimeIntegratorORK_SH_MPI_DGV(f,dt)
!!! END OF the MPI FORK

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! DIAGNOSTIC B (use together with A)  call to record a derivatives of macroparameters and the relaxation times of macroparameters. 
! The function will use the updated frhs1 directly from commvar and the old f can be found in f1; 
! The time corresponds to the initial time step. 
!!!!
call RecordMomDerivRelaxTimes_DGV(e_time)
!!!!  
! END DIAGNOSTIC B (use together with A)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 
 e_time = e_time+dt ! advance the time
end do 
